                        ...
                        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginLeft: 51 * SCALE }}>
                            {
                                sv.options.option.sort((a,b)=>{return Number(a.index)-Number(b.index)}).map((ssv, ssi) => {
                                    ...
                                    if(sv.resourcetype == 1){
                                        optioncontent = (<Text style={{ fontSize: 18 * SCALE, marginLeft: 12 * SCALE }}>{ssv.content}</Text>);
                                    }else {
                                        let imageUrl = "file://" + this.props.audioPath + ssv.content;
                                        optioncontent = (<Image
                                            style={{ marginLeft: 12 * SCALE, width: 200 * SCALE, height: 200 * SCALE }}
                                            source={{uri:imageUrl}}
                                        ></Image>);
                                    }
