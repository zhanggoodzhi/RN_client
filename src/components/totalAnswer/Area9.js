...
class Area extends React.Component {
    ...
    render() {
        ...
        const questionList = area.questions.map((v, i) => {
            ...
            //         <Text style={{ fontSize: 18 * SCALE, color: '#555' }}>{sv.answers.answer.content}</Text>
            //     </View>
            // );
            let answers;
            ...
            const imageUrl = "file://" + this.props.audioPath + sv.image;
            const picture = <FixedWidthImage width={752 * SCALE} imageUrl={imageUrl}>
            </FixedWidthImage>;
            return (
                <View key={i} style={{ marginVertical: 30 * SCALE }}>
                    <View style={{ flexDirection: 'row' }}>
                        <ImageBackground style={{ justifyContent: 'center', width: 39 * SCALE, height: 26 * SCALE, marginLeft: 15 * SCALE }}
            ...
            return (
                <View key={i} style={{ paddingHorizontal: 14 * SCALE }}>
                    {
                        v.prompt ? <Text style={{ marginTop: 25 * SCALE }}>{v.prompt}</Text> : null
                    }
