                    ...
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                        {
                            v.options.option.sort((a,b)=>{return Number(a.index)-Number(b.index)}).map((sv, si) => {
                                ...
                                if(v.resourcetype == 1){
                                    optioncontent = (<Text style={{ fontSize: 18 * SCALE, marginLeft: 12 * SCALE }}>{sv.content}</Text>);
                                }else if(v.resourcetype == 2){
                                ...
                                return (
                                    <View key={sv + '-' + si} style={{ marginRight: si === v.options.option.length - 1 ? 0 : 65 * SCALE, marginTop: 20 * SCALE,flex:1}}>
                                        <View
